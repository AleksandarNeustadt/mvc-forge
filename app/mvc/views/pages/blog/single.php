...
<div class="w-full max-w-5xl mx-auto pt-3 pb-4">
        <?php if (isset($blogPost) && !empty($blogPost)): ?>
            ...
            <article class="bg-slate-900/50 rounded-lg border border-slate-700 p-6 md:p-8 xl:p-10">
                ...
                <h1 class="text-3xl md:text-4xl font-bold text-white mb-4">
                    <?= htmlspecialchars($blogPost['title'] ?? 'Untitled Post') ?>
                </h1>

                <!-- Meta Information -->
                <div class="flex flex-wrap items-center gap-4 text-slate-400 text-sm mb-6 pb-6 border-b border-slate-700">
                    <?php if (!empty($blogPost['author'])): ?>
                        <div class="flex items-center">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                            </svg>
                            <?= htmlspecialchars($blogPost['author']['name'] ?? $blogPost['author']['username'] ?? 'Unknown') ?>
                        </div>
                    <?php endif; ?>
                    
                    <?php if (!empty($blogPost['published_at'])): ?>
                        <div class="flex items-center">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <?php if (!empty($blogPost['featured_image'])): ?>
                    <div class="mb-6">
                        <img src="<?= htmlspecialchars($blogPost['featured_image']) ?>" 

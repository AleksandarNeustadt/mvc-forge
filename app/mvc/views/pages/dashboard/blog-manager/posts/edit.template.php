...
@php
// Read old input from session BEFORE FormBuilder clears it
if (session_status() === PHP_SESSION_NONE) {
    session_start();
}
$oldInput = $_SESSION['_old_input'] ?? [];

// Ensure post is an array
...
// Get values - prefer old input (validation error), otherwise use post data
$contentValue = $oldInput['content'] ?? $post['content'] ?? '';
$currentFeaturedImage = $oldInput['featured_image'] ?? $post['featured_image'] ?? '';
$publishedAtValue = '';
if (!empty($post['published_at'])) {
    $publishedAt = is_int($post['published_at']) ? $post['published_at'] : strtotime($post['published_at']);
    $publishedAtValue = date('Y-m-d\TH:i', $publishedAt);
} elseif (!empty($oldInput['published_at'])) {
    $publishedAtValue = $oldInput['published_at'];
}

$hasContentError = class_exists('Form') && method_exists('Form', 'hasError') && Form::hasError('content');
$contentError = $hasContentError && method_exists('Form', 'getError') ? Form::getError('content') : '';
$hasFeaturedImageError = class_exists('Form') && method_exists('Form', 'hasError') && Form::hasError('featured_image');
$featuredImageError = $hasFeaturedImageError && method_exists('Form', 'getError') ? Form::getError('featured_image') : '';
...
<div class="p-8 max-w-4xl">
    ...
    <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6">
        @php
        $form = new FormBuilder("/{$lang}/dashboard/blog/posts/{$postId}", 'PUT');
        ...
        if (!empty($languagesData)) {
            ...
            $selectedLangId = class_exists('Form') && method_exists('Form', 'old') 
                ? Form::old('language_id', $post['language_id'] ?? '') 
                : ($post['language_id'] ?? '');
        ...
        $form->raw('<textarea id="content" name="content" class="w-full px-4 py-2 bg-slate-900/50 border border-slate-700 rounded-lg text-white focus:border-theme-primary focus:ring-1 focus:ring-theme-primary" rows="15" placeholder="Full post content">' . htmlspecialchars($contentValue) . '</textarea>');
        if ($hasContentError && $contentError) {
            $form->raw('<p class="text-red-400 text-sm mt-1">' . htmlspecialchars($contentError) . '</p>');
        }
        $form->raw('</div>');
        ...
        $form->raw('<input type="hidden" name="featured_image" id="featured_image_url" value="' . htmlspecialchars($currentFeaturedImage) . '">');
        $form->raw('<div id="featured_image_preview_container" class="' . ($currentFeaturedImage ? '' : 'hidden') . '">');
        $form->raw('<div class="relative inline-block">');
        $form->raw('<img id="featured_image_preview" src="' . htmlspecialchars($currentFeaturedImage) . '" alt="Featured Image Preview" class="max-w-md h-auto rounded-lg border border-slate-700">');
        ...
        $form->raw($currentFeaturedImage ? 'Change Featured Image' : 'Upload Featured Image');
        $form->raw('</label>');
        ...
        $form->raw('</div>');
        if ($hasFeaturedImageError && $featuredImageError) {
            $form->raw('<p class="text-red-400 text-sm mt-1">' . htmlspecialchars($featuredImageError) . '</p>');
        }
        $form->raw('</div>');
        $form->raw('</div>');
        ...
        $form->raw('<div id="published_at_field">');
        $form->raw('<label class="block text-sm font-medium text-slate-300 mb-2">Published At</label>');
        $form->raw('<input type="datetime-local" name="published_at" value="' . htmlspecialchars($publishedAtValue) . '" class="w-full px-4 py-2 bg-slate-900/50 border border-slate-700 rounded-lg text-white focus:border-theme-primary focus:ring-1 focus:ring-theme-primary">');
        $form->raw('</div>');
        ...
        if (empty($categories)) {
            ...
            foreach ($categories as $catId => $catName) {
                ...
                $form->raw('<input type="checkbox" name="categories[]" value="' . htmlspecialchars($catId) . '" id="category_' . $catId . '" class="w-4 h-4 text-theme-primary bg-slate-900 border-slate-700 rounded focus:ring-theme-primary"' . $checked . '>');
...
@php
$languagesDataJson = !empty($languagesData) ? json_encode($languagesData) : '{}';
@endphp
...
function filterCategoriesByLanguage(languageId) {
    ...
    const languagesData = {!! $languagesDataJson !!};
    ...
    {{-- Store currently selected category IDs before updating --}}
    const selectedCategoryIds = [];
    const categoryCheckboxes = categoriesContainer.querySelectorAll('input[type="checkbox"]');
    
    categoryCheckboxes.forEach(cb => {
        if (cb.checked) {
            selectedCategoryIds.push(parseInt(cb.value));
        }
    });
    ...
    fetch(categoriesUrl)
        ...
        .then(data => {
            if (data.success && data.data) {
                updateCategoriesList(data.data, selectedCategoryIds);
            }
        })
        .catch(error => {
...
function updateCategoriesList(categories, selectedIds) {
    ...
    categories.forEach(cat => {
        ...
        {{-- Restore checked state if it was selected before --}}
        if (selectedIds && selectedIds.includes(cat.id)) {
        ...
        tinymce.init({
            ...
            file_picker_callback: function (callback, value, meta) {
                if (meta.filetype === 'image') {
                    ...
                    // Use addEventListener instead of inline onchange (CSP-compliant)
...
@php
$tinymceApiKey = Env::get('TINYMCE_API_KEY', 'no-api-key');
@endphp

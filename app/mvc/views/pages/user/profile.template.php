        ...
        <div class="flex flex-col md:flex-row items-start gap-6">
            ...
            <div class="flex-1 w-full">
                <div class="mb-4">
                    ...
                    <p class="text-slate-400">@{{ $profileUser['username'] ?? '' }}</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <p class="text-white">
                            ...
                            @if (empty($profileUser['email_verified_at']))
                    ...
                    <div>
                        ...
                        <p class="text-white">
                            @if (!empty($profileUser['created_at']))
                                {{ date('F Y', is_int($profileUser['created_at']) ? $profileUser['created_at'] : strtotime($profileUser['created_at'])) }}
                            @else
                                N/A
                            @endif
                        </p>
                    ...
                    <div>
                        ...
                        <p class="text-white">
                            @if (!empty($profileUser['last_login_at']))
                                {{ date('Y-m-d H:i', is_int($profileUser['last_login_at']) ? $profileUser['last_login_at'] : strtotime($profileUser['last_login_at'])) }}
                            @else
                                Never
                            @endif
                        </p>
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <p class="text-white">
                        @if (!empty($profileApiToken['expires_at']))
                            {{ date('Y-m-d H:i', is_int($profileApiToken['expires_at']) ? $profileApiToken['expires_at'] : strtotime((string) $profileApiToken['expires_at'])) }}
                        @else
                            Never
                        @endif
                    </p>
                ...
                <div>
                    ...
                    <p class="text-white">
                        @if (!empty($profileApiToken['last_used_at']))
                            {{ date('Y-m-d H:i', is_int($profileApiToken['last_used_at']) ? $profileApiToken['last_used_at'] : strtotime((string) $profileApiToken['last_used_at'])) }}
                        @else
                            Never
                        @endif
                    </p>

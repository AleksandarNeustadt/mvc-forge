                        ...
                        @foreach ($table['columns'] as $column)
                            <tr class="hover:bg-slate-700/30 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @endphp
                                    @if ($keyType === 'PRI')
                                        <span class="px-2 py-1 bg-yellow-500/20 text-yellow-400 rounded text-xs font-medium">PRIMARY</span>
                                    @elseif (!empty($keyType))
                                        <span class="px-2 py-1 bg-blue-500/20 text-blue-400 rounded text-xs font-medium">{{ e($keyType) }}</span>
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    <form action="{{ route('dashboard.column.drop', ['table' => $table['name'], 'column' => $column['name']]) }}" 
                                          method="POST" 
                                          class="inline delete-column-form"
                                          data-action="delete"
                                          data-message="Are you sure you want to delete this column? This action cannot be undone.">
...
document.addEventListener('DOMContentLoaded', function() {
    document.addEventListener('submit', function(e) {
        const form = e.target;
        if (form.classList.contains('delete-column-form')) {

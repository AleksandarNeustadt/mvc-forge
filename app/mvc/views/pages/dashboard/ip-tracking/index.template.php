        ...
        <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-white">{{ number_format($totalRequests) }}</p>
                </div>
        ...
        <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-red-400">{{ number_format($suspiciousCount) }}</p>
                </div>
                        ...
                        @foreach (array_slice($ipStats, 0, 20) as $stat)
                            <tr class="hover:bg-slate-800/30 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-400">
                                    @if (!empty($stat['last_seen']))
                                        {{ date('Y-m-d H:i:s', is_int($stat['last_seen']) ? $stat['last_seen'] : strtotime($stat['last_seen'])) }}
                                    @else
                                        N/A
                                    @endif
                                </td>
                ...
                <tbody class="divide-y divide-slate-700/50">
                    ...
                    @else
                        @foreach ($recentEntries as $entry)
                            @php
                            $isSuspicious = $entry['is_suspicious'] ?? false;
                            $entryIp = $entry['ip_address'] ?? '';
                            $knownUsers = $ipUserMap[$entryIp] ?? [];
                            @endphp
                            <tr class="hover:bg-slate-800/30 transition-colors {{ $isSuspicious ? 'bg-red-900/10' : '' }}">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-400">
                                    @if (!empty($entry['created_at']))
                                        {{ date('Y-m-d H:i:s', is_int($entry['created_at']) ? $entry['created_at'] : strtotime($entry['created_at'])) }}
                                    @else
                                        N/A
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <code class="text-sm text-theme-primary font-mono">{{ e($entryIp) }}</code>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    @if (!empty($entry['known_service']))
                                        <span class="inline-flex items-center gap-1 px-2 py-1 bg-blue-500/20 text-blue-300 rounded-md text-xs font-medium">
                                            <ion-icon name="cloud-outline" class="text-xs"></ion-icon>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-300">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <div class="flex flex-col">
                                            ...
                                            @endif
                                            @if (!empty($entry['language_code']) && !empty($entry['language_name']))
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-300">
                                    ...
                                    @elseif (!empty($knownUsers))
                                        <span class="text-slate-400 text-xs" title="Known users from this IP: {{ e(implode(', ', array_column($knownUsers, 'username'))) }}">
                                            Guest (known: {{ e(implode(', ', array_slice(array_column($knownUsers, 'username'), 0, 2))) }}{{ count($knownUsers) > 2 ? '...' : '' }})
                                        </span>
                                    @else
                                        <span class="text-slate-500">Guest</span>
                                    @endif
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @php
                                    $method = $entry['request_method'] ?? 'GET';
                                    $methodClass = $method === 'GET' ? 'bg-blue-900/50 text-blue-300' : 'bg-purple-900/50 text-purple-300';
                                    @endphp
                                    <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium {{ $methodClass }}">
                                        {{ e($method) }}
                                    </span>
                                </td>

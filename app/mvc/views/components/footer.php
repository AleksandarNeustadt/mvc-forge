    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        ...
        if (class_exists('NavigationMenu')) {
            ...
            if (!empty($footerMenus)) {
                echo '<div class="mb-4 pb-4 border-b border-slate-800/50">';
                echo '<div class="flex flex-wrap gap-6 justify-center">';
                foreach ($footerMenus as $menu) {
                    $menuItems = $menu->getMenuItems();
                    if (!empty($menuItems)) {
                        echo '<div class="flex flex-col gap-2">';
                        echo '<h3 class="text-sm font-semibold text-slate-300 mb-2">' . htmlspecialchars($menu->name) . '</h3>';
                        echo '<div class="flex flex-col gap-1">';
                        foreach ($menuItems as $menuPage) {
                            $url = '/' . $currentLang . $menuPage->route;
                            $title = htmlspecialchars($menuPage->title ?? '');
                            echo '<a href="' . $url . '" class="text-xs sm:text-sm text-slate-400 hover:text-theme-primary transition-colors">' . $title . '</a>';
                        }
                        echo '</div>';
                        echo '</div>';
                    }
